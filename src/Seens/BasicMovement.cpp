...
void TestWorld::PhysicsUpdate(float MaxUpdateSpeed){
        ...
        if(TealBlockColision){
            TealBlockColision = BasicPhysics.QuadBodyColision(TealBlockPhysics, TealBlock->GetPhysicsPos(), PlayerPhysics, PlayerBlock->GetPhysicsPos(), 0.8f);
            ForceDirection NewTealBlockDirecton =  TealBlockColision.MovmentDirectionB;
            //TealBlockColision = BasicPhysics.QuadBodyColision(Player, PlayerPos, PhysicsTealBlock, TealBlockFuturePos, 0.3f);
            if(!TealBlockColision){
                TealBlockColision = BasicPhysics.QuadBodyColision(PlayerPhysics, PlayerBlock->GetPhysicsPos(), TealBlockPhysics, TealBlock->GetPhysicsPos(), 0.8f);
            ...
            if(TealBlockColision){
                ...
                TealBlockColision = BasicPhysics.AABBColision(TealBlockPhysics, m_ObjectWorldPositions[TealBlock->ObjectPositionFutureID], LandPhysics, m_ObjectWorldPositions[Land->ObjectPositionID]);
                if(TealBlockColision.IsColision){
                m_ObjectWorldPositions[TealBlock->ObjectPositionFutureID] = BasicPhysics.MovePhysicsObject(m_ObjectWorldPositions[TealBlock->ObjectPositionFutureID], TealBlockColision.MovmentDirectionB, PlayerMovmentSpeed + BasicPhysics.GetGravity().Speed);
                }
            }
